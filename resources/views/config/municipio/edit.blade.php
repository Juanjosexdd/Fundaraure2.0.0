    ...
    <div class="col-xs-12">
      <div class="card card-solid card-default">
        <div class="card-header with-border mailbox-controls">
          <h2 class="card-title"> <i class="fa fa-edit text-warning"></i> Editar datos de {{$municipio->nombre}}</h2>
          <a href="{{ route('municipio.index') }}" title="Volver" data-toggle="tooltip" data-placement="left" class="btn btn-default btn-sm elevation-1 float-right">&nbsp;&nbsp;<i class="fas fa-reply text-teal"></i>&nbsp;&nbsp;</a>
        </div>
        <div class="card-body">
          <div class="col-md-12">
            {!! Form::model($municipio, ['route' => ['municipio.update', $municipio->id],'method' => 'PUT'])!!}
              @method('PUT')
              @include('config.municipio.partials.form',['btnText'=>'Enviar'])
            {!! Form::close() !!}
          </div>
        </div>
      </div>
