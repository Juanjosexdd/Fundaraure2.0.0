<div class="btn-group m-0">
    <td>
        @can('config.tcliente.edit')
        <a title="Editar" href="{{route('tcliente.edit', $id)}}" data-toggle="tooltip" data-placement="top" title="Editar" class="btn btn-default" style="height: 32px; padding-bottom: 3px; padding-top: 3px;"><i class="fa fa-edit text-warning"></i></a>
        @endcan
    </td>
    <td>
        @can('config.tcliente.destroy')
        <form class=" inline form-eliminar" method="post" action="{{ route('tcliente.destroy', $id)}}">
            @csrf @method('DELETE')
