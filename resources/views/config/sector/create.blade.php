    ...
    <div class="col-xs-12">
      <div class="card">
        <div class="card-header with-border mailbox-controls">
          <h3 class="card-title"><i class="fa fa-edit text-warning"></i> Registrar sector</h3>
          <a href="{{ route('sector.index') }}" title="Volver" data-toggle="tooltip" data-placement="left" class="btn btn-default btn-sm elevation-1 float-right">&nbsp;&nbsp;<i class="fas fa-reply text-teal"></i>&nbsp;&nbsp;</a>
        </div>
        <div class="card-body">
          <div class="col-md-12">
            <form method="POST" action="{{ route('sector.store') }}">
              @csrf
              @include('config.sector.partials.form',['btnText'=>'Guardar'])
            </form>
          </div>
        </div>
      </div>
