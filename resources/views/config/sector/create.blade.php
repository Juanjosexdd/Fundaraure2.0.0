    ...
    <div class="col-xs-12">
      <div class="card">
        <div class="card-header with-border mailbox-controls">
          <h3 class="card-title">Registrar sector</h3>
          <div class="btn-group float-right">
            <a href="{{ route('sector.index')}}" type="button" title="Volver" data-toggle="tooltip" data-placement="left" class="btn btn-default btn-sm"><i class="fas fa-reply text-info"></i></a>
          </div>
        </div>
        <div class="card-body">
          <div class="col-md-8 offset-2">
            <form method="POST" action="{{ route('sector.store') }}">
              @csrf
              @include('sector.partials.form',['btnText'=>'Guardar'])
            </form>
          </div>
        </div>
      </div>
