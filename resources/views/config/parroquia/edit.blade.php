    ...
    <div class="col-xs-12">
      <div class="card card-solid card-default">
        <div class="card-header with-border mailbox-controls">
          <h2 class="card-title"> <i class="fa fa-edit text-warning"></i> Editar datos {{$parroquia->nombre}}</h2>
        ...
        <div class="card-body">
          <div class="col-md-8 offset-2">
            {!! Form::model($parroquia, ['route' => ['parroquia.update', $parroquia->id],'method' => 'PUT'])!!}
              @method('PUT')
              @include('parroquia.partials.form',['btnText'=>'Enviar'])
            {!! Form::close() !!}
          </div>
        </div>
      </div>
