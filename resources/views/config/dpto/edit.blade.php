...
@section('title','Editar Depatamento')
    ...
    <div class="col-xs-12">
      <div class="card card-solid card-default">
        <div class="card-header with-border mailbox-controls">
          <h2 class="card-title"> <i class="fa fa-edit text-warning"></i> Editar datos de {{$dpto->name}}</h2>
        ...
        <div class="card-body">
          <div class="col-md-12">
            {!! Form::model($dpto, ['route' => ['dpto.update', $dpto->id],'method' => 'PUT', 'files' => true])!!}
              @method('PUT')
              @include('config.dpto.partials.form',['btnText'=>'Enviar'])
            {!! Form::close() !!}
          </div>
        </div>
      </div>
